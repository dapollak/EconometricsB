...
merged_state_data$break_output <- merged_state_data$lnm_rate - dplyr::lag(merged_state_data$lnm_rate)

states_trend_break_full <- data.frame()
for (state_s in merged_state_data$state[!duplicated(merged_state_data$state)]) {
  for (tau in 1933:1940) {
    mmr_break_reg <- lm(
      break_output ~ I(year >= tau),
      data = filter(merged_state_data, state == state_s & disease == "mmr")
    )
    tryCatch(
      {
...
writeLines(print(xtable(states_trend_break_max)), "states_year_break.tex")
...
se_vals <- c()
p_vals <- c()
for (state_s in (merged_state_data %>% select(state) %>% distinct())$state) {
...
states_data <- data.frame(state=(subset_state_data_1925_1943_long %>%
                        select(state) %>%
                        distinct())$state)
...
writeLines(print(xtable(states_effects)), "table_did_weighted.tex")
...
writeLines(print(xtable(power_table)), "power.tex")
